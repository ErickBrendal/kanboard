#!/usr/bin/env python3
"""
EBL Kanboard → Power BI Sync Automático v3
Estratégia:
  1. Tenta buscar tarefas do Kanboard via API (com token e com user/pass)
  2. Se falhar, usa data_enriquecido.json (105 tarefas) como fonte principal
  3. Tenta autenticar no Power BI via ROPC; se MFA bloquear, tenta device code flow
  4. Se Power BI inacessível, salva CSV e log com status detalhado
Versão: 3.0 | 2026-04-05
"""

import msal, requests, json, csv, sys, os
from datetime import datetime
from pathlib import Path
from collections import Counter, defaultdict

# ===== CONFIGURAÇÕES =====
KANBOARD_URL   = "http://kanboard.eblsolucoescorp.tec.br/jsonrpc.php"
KANBOARD_USER  = "admin"
KANBOARD_PASS  = "Senha@2026"
KANBOARD_TOKEN = "a43a8785a4487979964cd7e12fc8c56bbb6ef7a6fa64bcb6c45fa1afc6ff"
KANBOARD_PROJ  = 11

PBI_USERNAME   = "admebl@eblsolucoescorporativas.com"
PBI_PASSWORD   = "Senha@2026"
PBI_TENANT_ID  = "208364c6-eee7-4324-ac4a-d45fe452a1bd"
PBI_CLIENT_ID  = "1950a258-227b-4e31-a9cf-717495945fc2"
PBI_CLIENT_ID2 = "04b07795-8ddb-461a-bbee-02f9e1bf7b46"  # Azure CLI client
PBI_DATASET_ID = "39d50fe5-cde9-4244-b5e5-422a73e8e142"
PBI_SCOPE      = ["https://analysis.windows.net/powerbi/api/.default"]

BASE_DIR       = Path(__file__).parent.parent
EXCEL_MAP      = BASE_DIR / "powerbi" / "excel_map.json"
DATA_ENRICH    = BASE_DIR / "powerbi" / "data_enriquecido.json"
LOCAL_SNAPSHOT = BASE_DIR / "backups" / "snapshot_antes_limpeza.json"
CSV_FINAL      = BASE_DIR / "powerbi" / "kanboard_dados_final.csv"
LOG_FILE       = BASE_DIR / "powerbi" / "sync_log.json"

SYNC_TS = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

def log(msg, level="INFO"):
    ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    print(f"[{ts}] [{level}] {msg}")

# =====================================================================
# AUTENTICAÇÃO POWER BI
# =====================================================================
def get_pbi_token_ropc(client_id):
    """Tenta autenticação ROPC (username/password)"""
    app = msal.PublicClientApplication(
        client_id,
        authority=f"https://login.microsoftonline.com/{PBI_TENANT_ID}"
    )
    result = app.acquire_token_by_username_password(
        PBI_USERNAME, PBI_PASSWORD, scopes=PBI_SCOPE
    )
    if "access_token" in result:
        return result["access_token"], None
    return None, result.get("error_description", result.get("error", "Desconhecido"))

def get_pbi_token():
    """Tenta autenticação no Power BI com múltiplas estratégias"""
    log("Autenticando no Power BI Service...")

    # Estratégia 1: ROPC com client_id principal
    log("  Tentativa 1: ROPC com client_id principal...")
    token, err = get_pbi_token_ropc(PBI_CLIENT_ID)
    if token:
        log("  Token obtido via ROPC (client_id principal)!")
        return token

    log(f"  Falhou: {err[:120] if err else 'erro desconhecido'}", "WARN")

    # Estratégia 2: ROPC com Azure CLI client_id
    log("  Tentativa 2: ROPC com Azure CLI client_id...")
    token, err = get_pbi_token_ropc(PBI_CLIENT_ID2)
    if token:
        log("  Token obtido via ROPC (Azure CLI client_id)!")
        return token

    log(f"  Falhou: {err[:120] if err else 'erro desconhecido'}", "WARN")

    # Se MFA está bloqueando, não há como fazer device code flow de forma não-interativa
    # Retornar None para indicar que o Power BI não está disponível
    log("  Power BI inacessível (MFA obrigatório). Continuando apenas com CSV local.", "WARN")
    return None

# =====================================================================
# BUSCA DE DADOS DO KANBOARD
# =====================================================================
def kanboard_api(method, params=None, auth=None, timeout=15):
    """Chama a API JSON-RPC do Kanboard"""
    payload = {"jsonrpc": "2.0", "method": method, "id": 1, "params": params or {}}
    r = requests.post(KANBOARD_URL, json=payload, auth=auth, timeout=timeout)
    r.raise_for_status()
    result = r.json()
    if "error" in result:
        raise Exception(f"API error: {result['error']}")
    return result.get("result")

def try_get_kanboard_tasks():
    """Tenta buscar tarefas do Kanboard com diferentes métodos de autenticação"""
    # Método 1: token como senha (Basic auth com user:token)
    for auth_pair in [
        (KANBOARD_USER, KANBOARD_TOKEN),
        ("jsonrpc", KANBOARD_TOKEN),
        (KANBOARD_USER, KANBOARD_PASS),
    ]:
        try:
            log(f"  Tentando Kanboard com auth {auth_pair[0]}:***...")
            open_tasks = kanboard_api("getAllTasks", {"project_id": KANBOARD_PROJ, "status_id": 1}, auth=auth_pair) or []
            closed_tasks = kanboard_api("getAllTasks", {"project_id": KANBOARD_PROJ, "status_id": 0}, auth=auth_pair) or []
            all_tasks = open_tasks + closed_tasks
            if all_tasks:
                log(f"  Kanboard OK: {len(open_tasks)} abertas + {len(closed_tasks)} fechadas = {len(all_tasks)} total")
                return all_tasks, "kanboard_api"
        except Exception as e:
            log(f"  Falhou: {e}", "WARN")

    return None, None

# =====================================================================
# CARREGAMENTO DE DADOS LOCAIS
# =====================================================================
def load_local_data():
    """Carrega dados locais enriquecidos (fonte principal de fallback)"""
    # Prioridade 1: data_enriquecido.json (105 tarefas completas)
    if DATA_ENRICH.exists():
        with open(DATA_ENRICH, 'r', encoding='utf-8') as f:
            data = json.load(f)
        log(f"  data_enriquecido.json: {len(data)} tarefas")
        return data, "data_enriquecido"

    # Prioridade 2: snapshot_antes_limpeza.json (97 tarefas brutas)
    if LOCAL_SNAPSHOT.exists():
        with open(LOCAL_SNAPSHOT, 'r', encoding='utf-8') as f:
            data = json.load(f)
        log(f"  snapshot_antes_limpeza.json: {len(data)} tarefas (brutas)")
        return data, "local_snapshot"

    raise Exception("Nenhuma fonte de dados disponível!")

# =====================================================================
# NORMALIZAÇÃO E ENRIQUECIMENTO
# =====================================================================
def normalize_enriched_tasks(data):
    """Normaliza tarefas do data_enriquecido.json para formato padrão"""
    log(f"Normalizando {len(data)} tarefas do data_enriquecido.json...")
    normalized = []
    for i, t in enumerate(data, 1):
        # Normalizar fase
        fase = t.get('fase', '01. Backlog')
        if 'Homog' in fase and fase != '08. Homologação':
            fase = '08. Homologação'

        # Normalizar prioridade
        pri_raw = str(t.get('pri', '0'))
        try:
            pri_num = int(float(pri_raw))
            if pri_num >= 3:
                pri = "Alta"
            elif pri_num == 2:
                pri = "Média"
            elif pri_raw in ["Alta", "Média", "Baixa"]:
                pri = pri_raw
            else:
                pri = "Média"
        except:
            pri = pri_raw if pri_raw in ["Alta", "Média", "Baixa"] else "Média"

        normalized.append({
            "seq":          t.get('seq', i),
            "cherwell":     str(t.get('cherwell', '')),
            "rdm":          str(t.get('rdm', '')),
            "titulo":       str(t.get('titulo', ''))[:200],
            "fase":         fase,
            "status":       t.get('status', 'Aberta'),
            "resp":         t.get('resp', 'Não atribuído'),
            "area":         t.get('area', ''),
            "tipo":         t.get('tipo', ''),
            "pri":          pri,
            "golive":       str(t.get('golive', '')),
            "previsao":     str(t.get('previsao', '')),
            "obs":          str(t.get('obs', ''))[:300],
            "valor":        float(t.get('valor', 0) or 0),
            "horas":        float(t.get('horas', 0) or 0),
            "dev":          str(t.get('dev', '')),
            "aprovado":     str(t.get('aprovado', '')),
            "aprovado_por": str(t.get('aprovado_por', '')),
            "requisitante": str(t.get('requisitante', '')),
            "valtech":      str(t.get('valtech', '')),
            "data_criacao": str(t.get('data_criacao', '')),
            "task_id":      str(t.get('task_id', '')),
            "sync_at":      SYNC_TS,
        })

    log(f"  {len(normalized)} tarefas normalizadas")
    return normalized

def enrich_from_kanboard(tasks):
    """Enriquece tarefas brutas do Kanboard com dados do Excel"""
    log(f"Enriquecendo {len(tasks)} tarefas do Kanboard com excel_map.json...")

    # Carregar excel_map
    excel_map = {}
    if EXCEL_MAP.exists():
        with open(EXCEL_MAP, 'r', encoding='utf-8') as f:
            excel_map = json.load(f)

    # Mapeamento de colunas hardcoded
    columns_cache = {
        "165": "01. Backlog", "166": "02. Refinamento", "167": "03. Priorizada",
        "168": "04. Análise", "169": "05. Estimativa", "170": "06. Aprovação",
        "171": "07. Desenvolvimento", "172": "08. Homologação", "173": "09. Deploy",
        "174": "10. Implementado", "175": "11. Cancelado",
        "176": "01. Backlog", "177": "02. Refinamento", "178": "04. Análise",
        "179": "05. Estimativa", "180": "06. Aprovação", "181": "07. Desenvolvimento",
        "182": "08. Homologação", "183": "09. Deploy", "184": "10. Implementado",
        "185": "11. Cancelado", "186": "01. Backlog", "187": "02. Refinamento",
        "188": "04. Análise", "189": "05. Estimativa", "190": "06. Aprovação",
        "191": "07. Desenvolvimento", "192": "08. Homologação", "193": "09. Deploy",
        "194": "10. Implementado", "195": "11. Cancelado", "39": "03. Priorizada"
    }

    enriched = []
    for i, task in enumerate(tasks, 1):
        title = task.get("title", "")
        parts = title.split(" - ", 2)
        if len(parts) >= 2:
            cherwell = parts[0].strip().lstrip('[').rstrip(']')
            rdm = parts[1].strip() if len(parts) > 2 else ""
            titulo = parts[-1].strip()
        else:
            cherwell = ""
            rdm = ""
            titulo = title

        excel_data = excel_map.get(cherwell, {})
        col_id = str(task.get("column_id", ""))
        col_name = columns_cache.get(col_id, "Backlog")

        # Mapear fase
        phase_map = {
            "Backlog": "01. Backlog", "Refinamento": "02. Refinamento",
            "Priorizada": "03. Priorizada", "Análise": "04. Análise",
            "Estimativa": "05. Estimativa", "Aprovação": "06. Aprovação",
            "Desenvolvimento": "07. Desenvolvimento", "Homologação": "08. Homologação",
            "Deploy": "09. Deploy", "Implementado": "10. Implementado",
        }
        fase = col_name
        for key, val in phase_map.items():
            if key.lower() in col_name.lower():
                fase = val
                break

        is_closed = task.get("is_active") in ("0", 0)
        status = "Implementado" if is_closed or "Implementado" in col_name else "Aberta"

        pri_num = int(task.get("priority", 0) or 0)
        if pri_num >= 3:
            pri = "Alta"
        elif pri_num == 2:
            pri = "Média"
        else:
            pri = excel_data.get("pri", "Média")

        due_date = task.get("date_due", "")
        if due_date and str(due_date) != "0":
            try:
                due_str = datetime.fromtimestamp(int(due_date)).strftime("%d/%m/%Y")
            except:
                due_str = str(due_date)
        else:
            due_str = excel_data.get("golive", "")

        created_at = task.get("date_creation", "")
        try:
            created_str = datetime.fromtimestamp(int(created_at)).strftime("%Y-%m-%d") if created_at else ""
        except:
            created_str = ""

        enriched.append({
            "seq":          i,
            "cherwell":     cherwell or f"KB-{task.get('id', '')}",
            "rdm":          rdm or excel_data.get("rdm", ""),
            "titulo":       titulo[:200],
            "fase":         fase,
            "status":       status,
            "resp":         excel_data.get("resp", "Não atribuído"),
            "area":         excel_data.get("area", ""),
            "tipo":         excel_data.get("tipo", ""),
            "pri":          pri,
            "golive":       due_str,
            "previsao":     excel_data.get("previsao", ""),
            "obs":          str(task.get("description", "") or "")[:300],
            "valor":        float(excel_data.get("valor", 0) or 0),
            "horas":        float(excel_data.get("horas", 0) or 0),
            "dev":          excel_data.get("dev", ""),
            "aprovado":     excel_data.get("aprovado", ""),
            "aprovado_por": excel_data.get("aprovado_por", ""),
            "requisitante": excel_data.get("requisitante", ""),
            "valtech":      excel_data.get("valtech", ""),
            "data_criacao": created_str,
            "task_id":      str(task.get("id", "")),
            "sync_at":      SYNC_TS,
        })

    log(f"  {len(enriched)} tarefas enriquecidas")
    return enriched

# =====================================================================
# CÁLCULO DE MÉTRICAS
# =====================================================================
def compute_kpis(tasks):
    total = len(tasks)
    impl = sum(1 for t in tasks if t["status"] == "Implementado")
    alta = sum(1 for t in tasks if t["pri"] == "Alta")
    backlog = sum(1 for t in tasks if t["fase"] == "01. Backlog")
    andamento = sum(1 for t in tasks if t["fase"] in [
        "02. Refinamento", "06. Aprovação", "07. Desenvolvimento", "09. Deploy"
    ])
    valor_total = sum(t["valor"] for t in tasks)
    horas_total = sum(t["horas"] for t in tasks)
    taxa = round(impl / total * 100, 1) if total > 0 else 0

    return [
        {"kpi": "Total de Demandas",  "valor": total,       "unidade": "demandas", "descricao": "Base Fast Track Salesforce"},
        {"kpi": "Implementadas",      "valor": impl,        "unidade": "demandas", "descricao": f"{taxa}% de conclusão"},
        {"kpi": "Em Andamento",       "valor": andamento,   "unidade": "demandas", "descricao": "Refin + Aprov + Dev + Deploy"},
        {"kpi": "Alta Prioridade",    "valor": alta,        "unidade": "demandas", "descricao": "Atenção imediata"},
        {"kpi": "No Backlog",         "valor": backlog,     "unidade": "demandas", "descricao": f"{round(backlog/total*100) if total else 0}% aguardando"},
        {"kpi": "Valor Total",        "valor": valor_total, "unidade": "R$",       "descricao": f"R$ {valor_total:,.0f}"},
        {"kpi": "Horas Estimadas",    "valor": horas_total, "unidade": "horas",    "descricao": "Valtech"},
        {"kpi": "Taxa de Conclusão",  "valor": taxa,        "unidade": "%",        "descricao": f"{impl} de {total} implementadas"},
    ]

def compute_fases(tasks):
    fases = Counter(t["fase"] for t in tasks)
    total = len(tasks)
    return [
        {"fase": f, "quantidade": v, "percentual": round(v / total * 100, 1)}
        for f, v in sorted(fases.items())
    ]

def compute_responsaveis(tasks):
    resp_data = defaultdict(lambda: {"total": 0, "implementadas": 0, "alta_prioridade": 0, "valor": 0.0, "horas": 0.0})
    for t in tasks:
        r = t["resp"]
        resp_data[r]["total"] += 1
        if t["status"] == "Implementado":
            resp_data[r]["implementadas"] += 1
        if t["pri"] == "Alta":
            resp_data[r]["alta_prioridade"] += 1
        resp_data[r]["valor"] += t["valor"]
        resp_data[r]["horas"] += t["horas"]

    result = []
    for resp, d in sorted(resp_data.items(), key=lambda x: -x[1]["total"]):
        taxa = round(d["implementadas"] / d["total"] * 100, 1) if d["total"] > 0 else 0
        result.append({
            "responsavel":    resp,
            "total":          d["total"],
            "implementadas":  d["implementadas"],
            "alta_prioridade": d["alta_prioridade"],
            "valor_total":    d["valor"],
            "horas_total":    d["horas"],
            "taxa_conclusao": taxa,
        })
    return result

# =====================================================================
# POWER BI: LIMPAR E INSERIR
# =====================================================================
def clear_pbi_table(token, table_name):
    headers = {"Authorization": f"Bearer {token}", "Content-Type": "application/json"}
    url = f"https://api.powerbi.com/v1.0/myorg/datasets/{PBI_DATASET_ID}/tables/{table_name}/rows"
    r = requests.delete(url, headers=headers, timeout=30)
    if r.status_code in [200, 204]:
        log(f"  Tabela '{table_name}' limpa com sucesso")
        return True
    else:
        log(f"  Aviso ao limpar '{table_name}': {r.status_code} - {r.text[:150]}", "WARN")
        return False

def push_rows(token, table_name, rows, batch_size=500):
    headers = {"Authorization": f"Bearer {token}", "Content-Type": "application/json"}
    url = f"https://api.powerbi.com/v1.0/myorg/datasets/{PBI_DATASET_ID}/tables/{table_name}/rows"
    total = len(rows)
    inserted = 0
    for i in range(0, total, batch_size):
        batch = rows[i:i + batch_size]
        payload = json.dumps({"rows": batch}, ensure_ascii=False)
        r = requests.post(url, data=payload.encode('utf-8'), headers=headers, timeout=60)
        if r.status_code == 200:
            inserted += len(batch)
            log(f"  Lote {i // batch_size + 1}: {len(batch)} linhas inseridas ({inserted}/{total})")
        else:
            log(f"  ERRO no lote {i // batch_size + 1}: {r.status_code} - {r.text[:200]}", "ERROR")
    return inserted

# =====================================================================
# SALVAR LOG
# =====================================================================
def save_log(sync_result):
    logs = []
    if LOG_FILE.exists():
        with open(LOG_FILE, 'r', encoding='utf-8') as f:
            try:
                logs = json.load(f)
            except:
                logs = []
    logs.append(sync_result)
    logs = logs[-30:]
    with open(LOG_FILE, 'w', encoding='utf-8') as f:
        json.dump(logs, f, ensure_ascii=False, indent=2)
    log(f"Log salvo: {LOG_FILE}")

# =====================================================================
# MAIN
# =====================================================================
def main():
    start = datetime.now()
    log("=" * 60)
    log("EBL Kanboard → Power BI Sync v3 Iniciado")
    log("=" * 60)

    sync_result = {
        "timestamp": start.isoformat(),
        "status": "error",
        "tasks_synced": 0,
        "kpis_synced": 0,
        "fases_synced": 0,
        "resp_synced": 0,
        "error": None,
        "data_source": "unknown",
        "powerbi_status": "not_attempted",
    }

    try:
        # ETAPA 1: Buscar dados
        log("\n[ETAPA 1] Buscando dados das tarefas...")
        tasks_raw, source = try_get_kanboard_tasks()

        if tasks_raw:
            log(f"Kanboard API disponível! {len(tasks_raw)} tarefas obtidas.")
            enriched = enrich_from_kanboard(tasks_raw)
            sync_result["data_source"] = "kanboard_api"
        else:
            log("Kanboard API indisponível. Usando dados locais enriquecidos...")
            local_data, local_source = load_local_data()
            enriched = normalize_enriched_tasks(local_data)
            sync_result["data_source"] = local_source

        log(f"Total de tarefas processadas: {len(enriched)}")

        # ETAPA 2: Calcular métricas
        log("\n[ETAPA 2] Calculando KPIs e métricas...")
        kpis = compute_kpis(enriched)
        fases = compute_fases(enriched)
        responsaveis = compute_responsaveis(enriched)

        log(f"  KPIs: {len(kpis)} | Fases: {len(fases)} | Responsáveis: {len(responsaveis)}")

        # Exibir resumo dos KPIs
        for kpi in kpis:
            log(f"  {kpi['kpi']}: {kpi['valor']} {kpi['unidade']}")

        # ETAPA 3: Salvar CSV
        log(f"\n[ETAPA 3] Salvando CSV em {CSV_FINAL}...")
        with open(CSV_FINAL, 'w', newline='', encoding='utf-8-sig') as f:
            if enriched:
                writer = csv.DictWriter(f, fieldnames=enriched[0].keys(), delimiter=';')
                writer.writeheader()
                writer.writerows(enriched)
        log(f"  CSV salvo: {len(enriched)} linhas")

        # ETAPA 4: Autenticar e sincronizar com Power BI
        log("\n[ETAPA 4] Sincronizando com Power BI...")
        token = get_pbi_token()

        if token:
            log("Limpando tabelas do Power BI...")
            for table in ["Demandas", "KPIs", "Fases", "Responsaveis"]:
                clear_pbi_table(token, table)

            log("Inserindo dados atualizados...")
            n_demandas = push_rows(token, "Demandas", enriched)
            n_kpis = push_rows(token, "KPIs", kpis)
            n_fases = push_rows(token, "Fases", fases)
            n_resp = push_rows(token, "Responsaveis", responsaveis)

            sync_result.update({
                "tasks_synced": n_demandas,
                "kpis_synced": n_kpis,
                "fases_synced": n_fases,
                "resp_synced": n_resp,
                "powerbi_status": "success",
            })
            log(f"Power BI atualizado: {n_demandas} demandas, {n_kpis} KPIs, {n_fases} fases, {n_resp} responsáveis")
        else:
            log("Power BI não sincronizado (MFA bloqueando autenticação automática)", "WARN")
            log("CSV local foi gerado com sucesso. Sincronização manual necessária.", "WARN")
            sync_result.update({
                "tasks_synced": len(enriched),
                "kpis_synced": len(kpis),
                "fases_synced": len(fases),
                "resp_synced": len(responsaveis),
                "powerbi_status": "mfa_blocked",
                "powerbi_note": "MFA obrigatório no tenant bloqueia autenticação automática ROPC. CSV local gerado com sucesso.",
            })

        elapsed = (datetime.now() - start).total_seconds()
        log(f"\n{'=' * 60}")
        log(f"SYNC CONCLUÍDO em {elapsed:.1f}s")
        log(f"  Fonte de dados: {sync_result['data_source']}")
        log(f"  Tarefas processadas: {len(enriched)}")
        log(f"  Power BI: {sync_result['powerbi_status']}")
        log(f"  CSV: {CSV_FINAL}")
        log(f"{'=' * 60}")

        sync_result.update({
            "status": "success",
            "elapsed_seconds": elapsed,
            "csv_path": str(CSV_FINAL),
            "total_tasks_local": len(enriched),
        })

    except Exception as e:
        log(f"ERRO CRÍTICO: {e}", "ERROR")
        import traceback
        traceback.print_exc()
        sync_result["error"] = str(e)
        save_log(sync_result)
        sys.exit(1)

    save_log(sync_result)
    return 0

if __name__ == "__main__":
    sys.exit(main())
