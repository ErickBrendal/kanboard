#!/usr/bin/env python3
"""
Corrige as 14 demandas com erro de importação e adiciona as faltantes
"""
...
TIPO_CAT = {
    "Parametrização": None,
    "Produtividade Comercial": None,
    "Produtividade TI": None,
    "Incidente ": None,
    "Incidente": None,
}
...
def get_existing_tasks():
    ...
    return {t["title"]: t["id"] for t in tasks}
...
def create_task(d, cats, lanes, existing):
    topico = str(d.get('Tópico') or '').strip()
    
    # Truncar título se necessário (max 255 chars)
    if len(topico) > 200:
        topico_display = topico[:197] + "..."
    else:
        topico_display = topico
    
    # Verificar se já existe
    if topico_display in existing or topico in existing:
        return "skip"
    ...
    cat_name = cat_map.get(tipo_clean, "Melhoria")
...
def main():
    print("=" * 60)
    print("CORREÇÃO DE ERROS E IMPORTAÇÃO COMPLETA")
    print("=" * 60)
    ...
    # Carregar Excel
    demandas = load_excel()
    print(f"Demandas no Excel: {len(demandas)}")
    
    # Importar apenas as faltantes
    created = 0
    skipped = 0
    errors = 0
    
    for d in demandas:
        result = create_task(d, cats, lanes, existing)
        if result == "skip":
            skipped += 1
        elif result:
            created += 1
            # Atualizar existing para evitar duplicatas na mesma execução
            topico = str(d.get('Tópico') or '').strip()
            if len(topico) > 200:
                topico = topico[:197] + "..."
            existing[topico] = result
        else:
            errors += 1
            topico = str(d.get('Tópico') or '').strip()
    ...
    print(f"\n📊 Total final no projeto: {len(all_tasks)}")
    ...
    resp_dist = {}
    for t in all_tasks:
        lid = t.get('swimlane_id')
        resp_name = lane_names.get(lid, f"Lane {lid}")
        resp_dist[resp_name] = resp_dist.get(resp_name, 0) + 1
    
    print("\n📊 Distribuição por responsável:")
    for resp, cnt in sorted(resp_dist.items(), key=lambda x: -x[1]):
        print(f"  {resp:25s}: {cnt:3d}")
